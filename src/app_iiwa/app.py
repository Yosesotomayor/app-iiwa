...
def get_desktop_dir() -> Path:
    ...
    if system == "Windows":
        try:
            ...
            windll.shell32.SHGetFolderPathW(None, CSIDL_DESKTOPDIRECTORY, None, SHGFP_TYPE_CURRENT, buf)
            p = Path(buf.value)
            if p.exists():
                return p
        except Exception:
...
def exportar_resumenes_en_grid(df_cps: dict, ruta_salida: str,
                                hoja="ResumenCPs", por_fila=4,
                                pad_filas=2, pad_cols=2):
    ...
    with pd.ExcelWriter(ruta_salida, engine="xlsxwriter") as writer:
        ...
        for i in range(0, len(items), por_fila):
            ...
            for j, (cp, df_bloque) in enumerate(fila_items):
                ...
                df_bloque.to_excel(
                    writer, sheet_name=hoja,
                    startrow=fila_actual + 1, startcol=col_inicio,
...
def run_proceso_campo(sistema_path: Path, data_dir: Path, output_dir: Path, log_func, month_label: str = "SISTEMA"):
    ...
    try:
        ...
        if "NumerodeCuenta" not in df.columns:
            ...
            df['NumerodeCuenta'] = df['Principal'].astype(str) + '-' + df['Derivada'].astype(str)
        ...
        required_cols = ["agua","actualizacionagua","recargosagua","drenaje",
                        "actualizaciondrenaje","recargosdrenaje","mejoras","iva"]
        ...
        df["Total"] = (
            df["agua"] + df["actualizacionagua"] + df["recargosagua"] +
            df["drenaje"] + df["actualizaciondrenaje"] + df["recargosdrenaje"] +
            df["mejoras"] + df["iva"]
        )
        ...
        t_consumo = pd.concat([
            t_consumo, 
            pd.DataFrame({"NumerodeCuenta":[t_consumo["NumerodeCuenta"].sum()]}, 
                        index=["Total general"])
        ])
        ...
        t_conexion = pd.concat([
            t_conexion, 
            pd.DataFrame({"NumerodeCuenta":[t_conexion["NumerodeCuenta"].sum()]}, 
                        index=["Total general"])
        ])
        ...
        for cps in codigos_postales:
            ...
            df_cps[f"{cps}"] = df_cp.groupby("TipoConexion")["NumerodeCuenta"].nunique().to_frame()
            ...
            df_cp["Total"] = df_cp["agua"] + df_cp["drenaje"] + df_cp["recargos"] + df_cp["mejoras"] + df_cp["iva"]
        ...
        with pd.ExcelWriter(tmp_cp_book, mode="w", engine="openpyxl") as writer:
            for cps in codigos_postales:
                ...
                det.to_excel(writer, sheet_name=hoja, index=False, startrow=0, startcol=0)
                startcol_resumen = det.shape[1] + 2
                res.to_excel(writer, sheet_name=hoja, startrow=0, startcol=startcol_resumen, index=True)
                
                ws = writer.sheets[hoja]
                ws.cell(row=1, column=startcol_resumen + 1, value="Resumen por TipoConexion")
...
def run_proceso_caja(sistema_path: Path, data_dir: Path, output_dir: Path, log_func):
    ...
    try:
        ...
        df_filtrado = df[(df['conDescripcion'] != 'MEJORAS AMBIENTALES') & (df['pagdAño'] < 2025)]
        df_filtrado.to_excel(output_dir / '2024-6_anteriores_y_sin_mejoras_ambientales.xlsx')

        # EVIDENCIAS-X fecha de pago
        log_func("[2/7] Calculando evidencias por fecha de pago…")
        def redondear(x): return round(x, 2)

        keys = ['FolioImpreso', 'fechapago']
        df['fechapago'] = pd.to_datetime(df['fechapago'], yearfirst=True).dt.strftime('%Y-%m-%d')
        df_filtrado['fechapago'] = pd.to_datetime(df_filtrado['fechapago'], yearfirst=True).dt.strftime('%Y-%m-%d')

        df['_cents'] = (pd.to_numeric(df['pagdCosto'], errors='coerce') * 100).round().astype('Int64')
        df_filtrado['_cents'] = (pd.to_numeric(df_filtrado['pagdCosto'], errors='coerce') * 100).round().astype('Int64')

        rezago_cents = (
            df_filtrado.groupby(keys, dropna=False, as_index=False)['_cents']
                    .sum().rename(columns={'_cents': '_rezago_cents'})
        )
        pago_total_cents = (
            df.groupby(keys, dropna=False, as_index=False)['_cents']
            .sum().rename(columns={'_cents': '_pago_cents'})
        )
        ...
        keys_all = (
            pd.concat([rezago_k[['k_folio','k_fecha']], pago_k[['k_folio','k_fecha']]], ignore_index=True)
            .drop_duplicates()
        )

        base = keys_all.merge(pago_k[['k_folio','k_fecha','_pago_cents']], on=['k_folio','k_fecha'], how='left') \
                    .merge(rezago_k[['k_folio','k_fecha','_rezago_cents']], on=['k_folio','k_fecha'], how='left')

        base['FolioImpreso'] = base['k_folio'].replace({'__NA__': np.nan})
        base['fechapago'] = base['k_fecha'].replace({'__NA__': np.nan})

        meta_cols = [
            'NumerodeCuenta', 'Propietario', 'Domicilio', 'Colonia', 'CodigoPostal',
            'AñoInicial', 'BimestreInicial', 'AñoFinal', 'BimestreFinal'
        ]
        meta = (df[keys + meta_cols].drop_duplicates(keys, keep='last'))
        evidencias_x_fecha = base.merge(meta, on=keys, how='left')

        evidencias_x_fecha['pago'] = (evidencias_x_fecha['_pago_cents'] / 100).astype(float)
        evidencias_x_fecha['REZAGO IIWA 2024-6 y anteriores (pagdCosto)'] = (
            evidencias_x_fecha['_rezago_cents'] / 100
        ).fillna(0.0).astype(float)
        evidencias_x_fecha['20% IIWA'] = evidencias_x_fecha['REZAGO IIWA 2024-6 y anteriores (pagdCosto)'] * 0.20

        orden_columnas = [
            'NumerodeCuenta', 'Propietario', 'Domicilio', 'Colonia', 'CodigoPostal',
            'AñoInicial', 'BimestreInicial', 'AñoFinal', 'BimestreFinal',
            'fechapago', 'FolioImpreso', 'pago',
            'REZAGO IIWA 2024-6 y anteriores (pagdCosto)', '20% IIWA'
        ]
        ...
        orden_pagos = ['DIAS', '# DE CUENTAS', 'PAGO CAJA',
                      'BASE IIWA 2024-6 Anteriores y sin Mejoras Ambientales',
                      'pagdDescuento', 'pagIva']

        pagos_diarios = df.groupby(['fechapago'], as_index=False).agg(
            {'FolioImpreso': 'nunique', 'pagdCosto': 'sum',
             'pagdDescuento': 'sum', 'pagIva': 'sum'}
        ).rename(columns={'fechapago': 'DIAS',
        ...
        pagos_diarios['DIAS'] = pd.to_datetime(pagos_diarios['DIAS']).dt.strftime('%d-%b')
        pagos_diarios.set_index('DIAS', inplace=True)

        base_por_dia = (
            df_filtrado
            .groupby(['fechapago'], as_index=False)['pagdCosto']
            .sum()
            .rename(columns={'pagdCosto': 'BASE IIWA 2024-6 Anteriores y sin Mejoras Ambientales'})
        )

        tmp = base_por_dia.copy()
        tmp['DIAS'] = pd.to_datetime(tmp['fechapago']).dt.strftime('%d-%b')
        tmp = tmp[['DIAS', 'BASE IIWA 2024-6 Anteriores y sin Mejoras Ambientales']].set_index('DIAS')

        pagos_diarios['BASE IIWA 2024-6 Anteriores y sin Mejoras Ambientales'] = (
            pagos_diarios.index.map(tmp['BASE IIWA 2024-6 Anteriores y sin Mejoras Ambientales'])
            .fillna(0.0)
        )
        ...
        pagos_x_cp = df.groupby(['CodigoPostal'], as_index=False).agg({
        ...
        base_por_cp = (
            df_filtrado
            .groupby(['CodigoPostal'], as_index=False)['pagdCosto']
            .sum()
            .rename(columns={'pagdCosto': 'BASE IIWA 2024-6 Anteriores y sin Mejoras Ambientales',
                             'CodigoPostal': 'C.P.'})
        )
        ...
        tmp = tmp[['C.P.', 'BASE IIWA 2024-6 Anteriores y sin Mejoras Ambientales']].set_index('C.P.')

        pagos_x_cp['BASE IIWA 2024-6 Anteriores y sin Mejoras Ambientales'] = (
            pagos_x_cp.index.map(tmp['BASE IIWA 2024-6 Anteriores y sin Mejoras Ambientales']).fillna(0.0)
        )
        pagos_x_cp['20% IIWA'] = pagos_x_cp['BASE IIWA 2024-6 Anteriores y sin Mejoras Ambientales'] * 0.20
        ...
        if registros_path.exists() and folios_path.exists():
            ...
            dict_folio_num.index = dict_folio_num.pop('NumerodeCuenta').astype(str).str.strip()

            evidencias_x_fecha.index = evidencias_x_fecha.pop('NumerodeCuenta')
            evidencias_x_fecha = evidencias_x_fecha.sort_values('fechapago', ascending=True)
            ...
            e_folio_geo['folio_notif'] = e_folio_geo.index.map(df_registros['folio_notif'].to_dict())
            
            evidencias_x_fecha.loc[inter, 'folio_notif'] = e_folio_geo['folio_notif']
            evidencias_x_fecha.insert(0, 'folio_notif', evidencias_x_fecha.pop('folio_notif'))
            ...
            evidencias_x_fecha['folio_notif'] = evidencias_x_fecha['folio_notif'].fillna(alt)

            resultado = evidencias_x_fecha.sort_index(kind='mergesort')

            log_func(f"Cuentas con folio notif: {len(resultado.loc[resultado['folio_notif'].notna()])}")
            log_func(f"Cuentas sin folio notif: {len(resultado.loc[resultado['folio_notif'].isna()])}")
            ...
            sin_geo['latitud_not'] = sin_geo.index.map(df_registros['latitud_not'].to_dict())
            sin_geo['longitud_not'] = sin_geo.index.map(df_registros['longitud_not'].to_dict())
            ...
            log_func("[5-6/7] Saltando procesamiento de REGISTROS/FOLIOS (archivos no encontrados)")
        ...
        with pd.ExcelWriter(salida_path, engine="openpyxl") as writer:
            ...
            for archivo in output_dir.glob("*.xlsx"):
                if archivo.name != "reporte_completo.xlsx" and not archivo.name.startswith("~"):
                    try:
...
class AppIIWA:
    ...
    def browse_sistema_file(self):
        ...
        file = filedialog.askopenfilename(title="Seleccionar archivo SISTEMA.xlsx",
